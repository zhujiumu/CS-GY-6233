...
int keys[NUM_KEYS];
pthread_mutex_t mutex[NUM_BUCKETS]; // initialize a mutex lock
...
int main(int argc, char **argv) {
  ...
  for (long cntr = 0; cntr < NUM_BUCKETS; cntr++)
      pthread_mutex_init(&mutex[cntr], NULL); //initialize mutex, we pass in address of mutex we declared at the beginning.
  ...
  for (cntr = 0; cntr < NUM_BUCKETS; cntr++)
      pthread_mutex_destroy(&mutex[cntr]); //destroy the mutex
