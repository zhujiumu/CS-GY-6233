...
int keys[NUM_KEYS];
pthread_spinlock_t spinlock[NUM_BUCKETS]; // initialize a spin lock
...
int main(int argc, char **argv) {
  ...
  long cntr;
  for (long cntr = 0; cntr < NUM_BUCKETS; cntr++)
      pthread_spin_init(&spinlock[cntr], 0); //initialize spinlock, we pass in address of spinlock we declared at the beginning.
  ...
  for (cntr = 0; cntr < NUM_BUCKETS; cntr++)
      pthread_spin_destroy(&spinlock[cntr]); //destroy the spinlock
